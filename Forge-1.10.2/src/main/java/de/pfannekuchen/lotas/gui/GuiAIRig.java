...
public class GuiAIRig extends GuiScreen {
	...
	public void initGui() {
		...
		xText = new GuiTextField(91, Minecraft.getMinecraft().fontRendererObj, width / 2 - 100, height - 50, 60, 20);
		xText.setText(spawnX + "");
		yText = new GuiTextField(92, Minecraft.getMinecraft().fontRendererObj, width / 2 - 30, height - 50, 60, 20);
		yText.setText(spawnY + "");
		zText = new GuiTextField(93, Minecraft.getMinecraft().fontRendererObj, width / 2 + 40, height - 50, 60, 20);
		...
		entities = mc.getIntegratedServer().worldServerForDimension(mc.thePlayer.dimension).getEntitiesWithinAABB(EntityLiving.class, mc.thePlayer.getEntityBoundingBox().expand(32, 32, 32));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		yText.drawTextBox();
		zText.drawTextBox();
		drawCenteredString(mc.fontRendererObj, entities.get(selectedIndex).getClass().getSimpleName().replaceFirst("Entity", "") + " (" + entities.get(selectedIndex).getPosition().getX() + ", " + entities.get(selectedIndex).getPosition().getY() + ", " + entities.get(selectedIndex).getPosition().getZ() + ")", width / 2, 5, 0xFFFFFF);
	}
