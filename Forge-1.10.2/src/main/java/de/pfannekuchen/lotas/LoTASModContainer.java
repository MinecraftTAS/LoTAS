...
@Mod(acceptedMinecraftVersions = "1.10.2", canBeDeactivated = false, clientSideOnly = true, modid = "lotas", name = "LoTAS", version = "${version}")
public class LoTASModContainer {
	...
	public void preinit(FMLPreInitializationEvent e) {
		new Thread(() -> {
			...
			try {
				BufferedReader stream = new BufferedReader(new InputStreamReader(new URL("http://mgnet.work/taschallenges/maps1.10.2.txt").openStream()));
	...
	public void loadSeeds() throws Exception {
		...
		try {
			...
			URL url = new URL("http://mgnet.work/seeds/seeds1.10.2.txt");
