...
public class OreDropManipulation extends GuiLootManipulation.DropManipulation {
    ...
    @Override
    public List<ItemStack> redirectDrops(Block blockstate) {
        if (blockstate == Blocks.lapis_ore && optimizeLapis.isChecked()) {
            return ImmutableList.of(new ItemStack(Items.dye, 9, 4));
        } else if (blockstate == Blocks.redstone_ore && optimizeRedstone.isChecked()) {
