...
public class GuiEntitySpawner extends GuiScreen {
	...
	public void initGui() {
		...
		if (mc.theWorld.difficultySetting == EnumDifficulty.HARD) {
		...
		buttonList.add(new GuiButton(8, width / 9 * 3 + 6, height - 24, width / 9 - 4, 20, "X++"));
		buttonList.add(new GuiButton(2, width / 9 * 4 + 6, height - 24, width / 9 - 4, 20, "X--"));
		buttonList.add(new GuiButton(5, width / 9 * 5 + 3, height - 24, width / 9 - 4, 20, "Y++"));
		buttonList.add(new GuiButton(9, width / 9 * 6 + 3, height - 24, width / 9 - 4, 20, "Y--"));
		...
		buttonList.add(new GuiButton(12, 5, height - 46, width / 3, 20, "Done"));
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton)  {
		...
		if (e instanceof EntityMob) {
			((GuiButton) buttonList.get(buttonList.size() - 2)).enabled = e.worldObj.difficultySetting != EnumDifficulty.PEACEFUL && ((EntityMob) e).getBlockPathWeight((int) e.posX, (int) e.boundingBox.minY, (int) e.posZ) >= 0.0F && isValidLightLevel(e);
		} else {
			((GuiButton) buttonList.get(buttonList.size() - 2)).enabled = true;
		}
	}
	...
	private boolean isValidLightLevel(EntityLiving entity) {
        if (entity.worldObj.getLightBrightness((int) entity.posX, (int)entity.boundingBox.minY, (int) entity.posZ) > 31) {
            return false;
        } else {
            int i = (int) entity.worldObj.getLightBrightness((int) entity.posX, (int)entity.boundingBox.minY, (int) entity.posZ);
            return i <= 7;
        }
	}
