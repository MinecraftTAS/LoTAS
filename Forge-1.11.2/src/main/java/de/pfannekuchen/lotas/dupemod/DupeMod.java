...
public class DupeMod {
	...
	public static void loadChests() {
		try {
			...
			for (TileEntity tileentity : DupeMod.tileentities) {
				mc.getIntegratedServer().worldServerForDimension(mc.player.dimension).getTileEntity(tileentity.getPos()).deserializeNBT(((TileEntityChest) tileentity).serializeNBT());
	...
	public static void saveItems() {
		try {
			...
			for (EntityItem item : mc.getIntegratedServer().worldServerForDimension(mc.player.dimension).getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(pX - 16, pY - 16, pZ - 16, pX + 16, pY + 16, pZ + 16))) {
	...
	public static void saveChests() {
		try {
			...
			Minecraft mc = Minecraft.getMinecraft();
			WorldServer world = mc.getIntegratedServer().worldServerForDimension(mc.player.dimension);
			...
			for (int x =- 5; x <= 5; x++) {
				for (int y =- 5; y <= 5; y++) {
					for (int z =- 5; z <= 5; z++) {
						if (mc.getIntegratedServer().worldServerForDimension(mc.player.dimension).getBlockState(playerPos.add(x, y, z)).getBlock() == Blocks.CHEST || world.getBlockState(playerPos.add(x, y, z)).getBlock() == Blocks.TRAPPED_CHEST) {
