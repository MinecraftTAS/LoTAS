...
public class GuiChallengeEscape extends GuiIngameMenu {
    ...
    public void initGui() {
		...
		double pY = Minecraft.getMinecraft().player.posY;
		double pZ = Minecraft.getMinecraft().player.posZ;
		DupeMod.trackedObjects = new ArrayList<EntityItem>();
        for (EntityItem item : Minecraft.getMinecraft().getIntegratedServer().worldServerForDimension(Minecraft.getMinecraft().player.dimension).getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(pX - 16, pY - 16, pZ - 16, pX + 16, pY + 16, pZ + 16))) {
    	...
    	try {
    		...
    		this.buttonList.get(this.buttonList.size() - 1).enabled = Minecraft.getMinecraft().theIntegratedServer.worldServerForDimension(mc.player.dimension).getEntities(EntityDragon.class, Predicates.alwaysTrue()).size() != 0;
    	} catch (Exception e) {
