...
public class DeadbushDropManipulation extends GuiLootManipulation.DropManipulation {
    ...
    public List<ItemStack> redirectDrops(IBlockState block) {
        if (block.getBlock().getDefaultState().getBlock() != Blocks.deadbush) return ImmutableList.of();
    ...
    public void render(int mouseX, int mouseY, float delta) {
        ...
        if (!enabled.isChecked()) {
            ...
            Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow("Drop " + sticks + " Sticks when breaking a Dead Bush", x, y + 64, 0xFFFFFF);
