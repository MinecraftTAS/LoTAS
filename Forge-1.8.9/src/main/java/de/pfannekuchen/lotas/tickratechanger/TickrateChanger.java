...
public class TickrateChanger {
	...
	public static void updateClientTickrate(int tickrateIn) {
		...
		tickrate = tickrateIn;
		if (!ConfigManager.getBoolean("ui", "hideTickrateMessages") && Minecraft.getMinecraft().ingameGUI != null) Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(new ChatComponentText("Updated Tickrate to \u00A7b" + tickrateIn));
	}
	...
	public void onDraw(RenderGameOverlayEvent.Post e) {
		if (e.type == ElementType.TEXT && Timer.ticks != -1) {
	...
	public void onInput(KeyInputEvent e) {
		if (Binds.slower.isPressed()) index++;
		else if (Binds.faster.isPressed()) index--;
		else return;
		index = MathHelper.clamp_int(index, 0, 10);
