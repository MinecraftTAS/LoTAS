...
public class AIManipulationScreen extends Screen {
	...
	public void init() {
		...
		addButton(new Button(5, 5, 98, 20, "<", btn -> {
		...
		addButton(new Button(width - 5 - 98, 5, 98, 20, ">", button -> {
			manip.selectNext();
			buttons.get(0).active = manip.hasPrevious();
			buttons.get(1).active = manip.hasNext();
			buttons.get(2).active = !manip.contains(AIManipMod.getSelectedEntity());
		...
		xText = new EditBox(Minecraft.getInstance().font, width / 2 - 98, height - 48, 58, 19, (int) target.x + "");
		yText = new EditBox(Minecraft.getInstance().font, width / 2 - 29, height - 48, 59, 19, (int) target.y + "");
		zText = new EditBox(Minecraft.getInstance().font, width / 2 + 39, height - 48, 59, 19, (int) target.z + "");

		addButton(new Button(width / 2 - 100, height - 25, 200, 20, "Change Target", button -> {
			manip.confirm();
			button.active=false;
		}));
		...
		addButton(new Button(width / 2 - 100, height - 72, 200, 20, "Move to me", btn -> {
			manip.setTargetToPlayer();
			setTextToVec(AIManipMod.getTargetPos());
		}));
		addButton(new Button(width / 2 - 100, height - 95, 200, 20, "Move to entity", btn -> {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(Minecraft.getInstance().font, AIManipMod.getSelectedEntity().getName().getString() + " (" + (int)entityPos.x + ", " + (int)entityPos.y + ", " + (int)entityPos.z + ")", width / 2, 5, 0xFFFFFF);
	}
