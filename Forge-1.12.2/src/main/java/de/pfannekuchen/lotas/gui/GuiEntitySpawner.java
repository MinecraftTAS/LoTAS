...
public class GuiEntitySpawner extends GuiScreen {
	...
	public void initGui() {
		...
		xText = new GuiTextField(91, Minecraft.getMinecraft().fontRenderer, width / 9 * 3 + 6, height - 46, (int) (width / 4.5) - 6, 20);
		xText.setText(spawnX + "");
		yText = new GuiTextField(92, Minecraft.getMinecraft().fontRenderer, width / 9 * 5 + 4, height - 46, (int) (width/ 4.5) - 6, 20);
		yText.setText(spawnY + "");
		zText = new GuiTextField(93, Minecraft.getMinecraft().fontRenderer, width / 9 * 7 + 2, height - 46, (int) (width/ 4.5) - 6, 20);
	...
	private boolean isValidLightLevel(EntityLiving entity) {
        ...
        if (entity.world.getLightFor(EnumSkyBlock.SKY, blockpos) > 31) {
            return false;
        } else {
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		case 12:
			...
			break;
		case 69:
			Minecraft.getMinecraft().integratedServer.getWorld(mc.player.dimension).spawnEntity(e);
		default:
			break;
		}
