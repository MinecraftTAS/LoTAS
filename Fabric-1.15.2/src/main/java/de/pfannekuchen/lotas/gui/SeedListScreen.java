...
public class SeedListScreen extends Screen {
    ...
    protected void init() {
        ...
        ButtonWidget create = new ButtonWidget(width / 2 - 100, height - 52, 200,20, "Create World", button -> {
            ...
            createWorldScreen.seed = selectedSeed.seed;
    ...
    protected void renderHoleBackground(int top, int bottom, int alphaTop, int alphaBottom) {
        ...
        bufferBuilder.vertex(0, bottom, 0.0F).texture(0.0F, ((float)bottom / 32.0F)).color(64, 64, 64, alphaBottom).next();
        bufferBuilder.vertex((0 + this.width), bottom, 0.0F).texture((this.width / 32.0F), ((float)bottom / 32.0F)).color(64, 64, 64, alphaBottom).next();
        bufferBuilder.vertex((0 + this.width), top, 0.0F).texture(((float)this.width / 32.0F), ((float)top / 32.0F)).color(64, 64, 64, alphaTop).next();
        bufferBuilder.vertex(0, top, 0.0F).texture(0.0F, ((float)top / 32.0F)).color(64, 64, 64, alphaTop).next();
        tessellator.draw();
    }
    ...
    public void drawSeeds() throws IOException {
        ...
        for (Seed seed : seeds) {
            if (selectedSeed == seed) {
                ...
                bufferBuilder.vertex(r-1, (y + n + 2), 1.0F).next();
                bufferBuilder.vertex(q-1, (y + n + 2), 1.0F).next();
                bufferBuilder.vertex(q-1, (y - 2), 1.0F).next();
                bufferBuilder.vertex(r-1, (y - 2), 1.0F).next();
                tessellator.draw();
                GlStateManager.color4f(0.0F, 0.0F, 0.0F, 0.5F);
                bufferBuilder.begin(7, VertexFormats.POSITION);
                bufferBuilder.vertex((r), (y + n + 1), 1.0F).next();
                bufferBuilder.vertex((q - 2), (y + n + 1), 1.0F).next();
                bufferBuilder.vertex((q - 2), (y - 1), 1.0F).next();
                bufferBuilder.vertex((r), (y - 1), 1.0F).next();
                tessellator.draw();
    ...
    public void render(int mouseX, int mouseY, float delta) {
        ...
        GlStateManager.color4f(1.0F, 1.0F, 1.0F, 1.0F);
        bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
        bufferBuilder.vertex(left, bottom, 0.0F).texture(((float)left / 32.0F), ((float)(bottom + (int)1) / 32.0F)).color(32, 32, 32, 255).next();
        bufferBuilder.vertex(right, bottom, 0.0F).texture(((float)right / 32.0F), ((float)(bottom + (int)1) / 32.0F)).color(32, 32, 32, 255).next();
        bufferBuilder.vertex(right, top, 0.0F).texture(((float)right / 32.0F), ((float)(top + (int)1) / 32.0F)).color(32, 32, 32, 255).next();
        bufferBuilder.vertex(left, top, 0.0F).texture(((float)left / 32.0F), ((float)(top + (int)1) / 32.0F)).color(32, 32, 32, 255).next();
        tessellator.draw();
        ...
        // Draw top gradient
        bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
        bufferBuilder.vertex(left, top + 4, 0.0F).texture(0.0F, 1.0F).color(0, 0, 0, 0).next();
        bufferBuilder.vertex(right, top + 4, 0.0F).texture(1.0F, 1.0F).color(0, 0, 0, 0).next();
        bufferBuilder.vertex(right, top, 0.0F).texture(1.0F, 0.0F).color(0, 0, 0, 255).next();
        bufferBuilder.vertex(left, top, 0.0F).texture(0.0F, 0.0F).color(0, 0, 0, 255).next();
        tessellator.draw();

        // Draw bottom gradient
        bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
        bufferBuilder.vertex(left, bottom, 0.0F).texture(0.0F, 1.0F).color(0, 0, 0, 255).next();
        bufferBuilder.vertex(right, bottom, 0.0F).texture(1.0F, 1.0F).color(0, 0, 0, 255).next();
        bufferBuilder.vertex(right, bottom - 4, 0.0F).texture(1.0F, 0.0F).color(0, 0, 0, 0).next();
        bufferBuilder.vertex(left, bottom - 4, 0.0F).texture(0.0F, 0.0F).color(0, 0, 0, 0).next();
        tessellator.draw();
