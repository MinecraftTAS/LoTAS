...
public class SavestatesLoWidget extends WindowLoWidget {
	...
	public SavestatesLoWidget() {
		super("savestatewidget", Component.literal("Savestates"), .237, .047);
	}
	...
	protected void render(PoseStack stack, double curX, double curY) {
		...
		if (this.animationProgress != 6) {
			stack.translate(this.vertical ? (this.leftORight ? -1000 : 1000) + this.ease(this.animationProgress, 0, 1, 6) * (this.leftORight ? +1000 : -1000) : 0, this.horizontal ? (this.topOBottom ? -1000 : 1000) + this.ease(this.animationProgress, 0, 1, 6) * (this.topOBottom ? +1000 : -1000) : 0, 0);
		}
		...
		for (int i = 0; i < Math.min(LoTAS.savestatemod.getStateCount(), 6); i++) {
			...
			// Render info
			this.draw(stack, Component.literal(s.getName()), this.x + 0.1, this.y + i * 0.1 + 0.045, 20, 0xff8f8f8f, false);
			this.draw(stack, Component.literal(new SimpleDateFormat().format(Date.from(Instant.ofEpochSecond(s.getTimestamp())))), this.x + 0.1, this.y + i * 0.1 + 0.065, 20, 0xff8f8f8f, false);
			// Render button
			if (curX > this.x + 0.095 && curY > this.y + 0.095 + i * 0.1 && curX < this.x + 0.16 && curY < this.y + 0.09 + 0.035 + i * 0.1) {
				this.fill(stack, this.x + 0.095, this.y + 0.095 + i * 0.1, this.x + 0.16, this.y + 0.09 + 0.035 + i * 0.1, 0xff149b5b);
				this.draw(stack, Component.literal("Loadstate"), this.x + 0.095 + 0.0065, this.y + 0.1 + i * 0.1, 20, i % 2 != 0 ? 0xff1b1c21 : 0xff0a0a0b, false);
			} else {
				this.fill(stack, this.x + 0.095, this.y + 0.095 + i * 0.1, this.x + 0.16, this.y + 0.09 + 0.035 + i * 0.1, i % 2 != 0 ? 0xff1b1c21 : 0xff0a0a0b);
				this.draw(stack, Component.literal("Loadstate"), this.x + 0.095 + 0.0065, this.y + 0.1 + i * 0.1, 20, 0xff149b5b, false);
			}

			if (curX > this.x + 0.164 && curY > this.y + 0.095 + i * 0.1 && curX < this.x + 0.23 && curY < this.y + 0.09 + 0.035 + i * 0.1) {
				this.fill(stack, this.x + 0.164, this.y + 0.095 + i * 0.1, this.x + 0.23, this.y + 0.09 + 0.035 + i * 0.1, 0xff149b5b);
				this.draw(stack, Component.literal("Deletestate"), this.x + 0.095 + 0.0724, this.y + 0.1 + i * 0.1, 20, i % 2 != 0 ? 0xff1b1c21 : 0xff0a0a0b, false);
			} else {
				this.fill(stack, this.x + 0.164, this.y + 0.095 + i * 0.1, this.x + 0.23, this.y + 0.09 + 0.035 + i * 0.1, i % 2 != 0 ? 0xff1b1c21 : 0xff0a0a0b);
				this.draw(stack, Component.literal("Deletestate"), this.x + 0.095 + 0.0724, this.y + 0.1 + i * 0.1, 20, 0xff149b5b, false);
			}
			// Render image
